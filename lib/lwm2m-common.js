  ...
  buildResponse() {
    ...
    Object.keys(this.resources).forEach((key) => {
      ...
      if (r instanceof Resource) {
        if (ACL.isAllowed(r.acl, ACL.READABLE)) {
          try {
            res = r.serialize(res);
          } catch (e) {
            this.setStatus(e.status || COAP_ERROR.COAP_404_NOT_FOUND);
          }
        } else {
          this.setStatus(COAP_ERROR.COAP_405_METHOD_NOT_ALLOWED);
        }
      } else {
        this.client.error(`Unexpected resource type: ${typeof(r)}, r=>${r}`);
        this.setStatus(COAP_ERROR.COAP_500_INTERNAL_SERVER_ERROR);
      }
    });
