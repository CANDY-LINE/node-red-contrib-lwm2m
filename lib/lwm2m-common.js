...
const LWM2M_TYPE = {
  ...
  toType(input) {
    ...
    let value = LWM2M_TYPE[input];
    if (!value) {
      value = LWM2M_TYPE.UNDEFINED;
    }
    return value;
  },
  toString(input) {
    let value = parseInt(input);
    let label = Object.keys(LWM2M_TYPE)[value];
    if (!label) {
      return 'UNDEFINED';
    }
    return label;
    ...
    switch (this.type) {
      ...
      case LWM2M_TYPE.OPAQUE:
        if (Buffer.isBuffer(this.value)) {
          ...
          throw new Error(`The value doesn't match the expected data type:${this.type} != Buffer`);
        }
        break;
