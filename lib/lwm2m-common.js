...
export class Resource {
  ...
  /* Promise */ update(/* any object */ newValue) {
    if (this.isExecutable()) {
    ...
    if (!ACL.isAllowed(this.acl, ACL.WRITABLE)) {
      return Promise.reject({
        status: COAP_ERROR.COAP_405_METHOD_NOT_ALLOWED,
        message: `Method Not Allowed`
      });
    ...
    switch (typeof(resource)) {
      ...
      default:
        if (Buffer.isBuffer(resource)) {
          ...
          return Promise.all((resource.value || []).map((r) => Resource.from(r))).then((values) => {
            return Resource.build({
              type: 'MULTIPLE_RESOURCE',
              acl: resource.acl,
...
export class ResourceRepositoryBuilder {
  constructor(json={}) {
    ...
    let jsonIds = Object.keys(this.json);
    let defaultIds = Object.keys(DEFAULT_REPO).filter(id => !jsonIds.includes(id));
    defaultIds.forEach((id) => {
      this.json[id] = DEFAULT_REPO[id];
    });
  }
...
export class LwM2MClientProxy extends EventEmitter {
  ...
  constructor(opts={}, objectStore={}) {
    ...
    this.log = opts.log.bind(opts) || console.log;
    this.error = opts.error.bind(opts) || console.error;
    this.objectStore = objectStore;
    this.autoReconnect = true;
    this.serverId = opts.serverId || 99;
  }
...
export class LwM2MObjectStore {
  ...
  write(uri, /* any object */ value, /* boolean */ remote) {
    if (uri && this.repo[uri]) {
      return this.repo[uri].update(value).then(() => {
        if (this.updatedUris.indexOf(uri) < 0) {
          this.updatedUris.push(uri);
        }
        this.emit(uri, this.repo[uri], 'updated', remote);
        return Promise.resolve();
      }).catch((err) => {
        if (!remote) {
          err.uri = uri;
