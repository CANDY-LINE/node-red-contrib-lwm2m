...
class Resource {
  ...
  constructor(id, type=LWM2M_TYPE.UNDEFINED, acl=ACL.READABLE, value=null) {
    if (typeof(id) === 'object') {
      this.id = id.id;
      this.type = LWM2M_TYPE.toType(id.type);
      this.acl = id.acl || ACL.READABLE;
      this.value = id.value;
    } else {
      this.id = id;
      this.type = LWM2M_TYPE.toType(type);
      this.acl = acl || ACL.READABLE;
      this.value = value;
    }
    ...
    if (init) {
      try {
        this.value = Resource.from(init.apply(this)).value;
      } catch (_) {
        delete this.value;
      }
    }
  ...
  update(/* any object */ newValue) {
    ...
    if (this.value && typeof(this.value.set) === 'function') {
      ...
      if (!(newValue instanceof Resource)) {
        newValue = Resource.from(newValue);
      }
