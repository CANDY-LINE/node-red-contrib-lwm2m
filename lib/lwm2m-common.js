...
export class RequestHandler {
  ...
  constructor(client, command, payload=Buffer.from([])) {
    this.client = client;
...
class Notify extends RequestHandler {
  ...
  perform() {
    let ev = Notify.STATE_TABLE[this.stateLabel];
    if (ev) {
      this.client.emit(ev);
    }
    return Promise.resolve({
...
RequestHandler.build = (client, command, payload) => {
  switch (command) {
    ...
    // not a lwm2m command
    case 'notify':
      return new Notify(client, command, payload);
    default:
