  ...
  serialize(/* Buffer */ input=Buffer.from([])) {
    ...
    switch (this.type) {
      ...
      case LWM2M_TYPE.OPAQUE:
        ...
        break;
      case LWM2M_TYPE.BOOLEAN:
        const booleanVal = resolveValue(value, []);
        if (Array.isArray(booleanVal)) {
          value = Buffer.from(booleanVal.length < 2 ? booleanVal : [booleanVal[0] ? 1 : 0]);
        } else {
...
class ReadInstances {
  ...
  perform() {
    return this.client.objectStore.remoteGet(this.uris).then((resources) => {
      ...
      this.client.error(`[ReadInstances] (${this.uris}) Error Message:${err.message || ''}`, { payload: err });
      this.setStatus(err.status);
      return this.sendResponse();
    });
  }
  ...
  constructor(opts={}) {
    ...
    this.on('connected', () => {
      if (this.requestBootstrap && this.saveProvisionedConfig) {
        ...
        this.debug(`<stdout> [saveProvisionedConfig=>false] Deleting credentials file => ${done ? 'OK' : 'FAIL'}!`);
      }
