...
const LWM2M_CRED_FILE_NAME = 'lwm2m_cred.json';
...
class CredUtils {
  ...
  static decryptCredentials(key, enc) {
      const value = enc.$;
  ...
  static encryptCredentials(key, credentials) {
  ...
  static loadCredentials(userDir, key) {
    if (!userDir || !key) {
      return null;
    }
    try {
      const data = fs.readFileSync(`${userDir}/${LWM2M_CRED_FILE_NAME}`, 'utf-8');
  ...
  static saveCredentials(userDir, key, credentials) {
    if (!userDir || !key) {
      return false;
    }
    const cred = CredUtils.encryptCredentials(key, credentials);
    fs.writeFileSync(`${userDir}/${LWM2M_CRED_FILE_NAME}`, JSON.stringify(cred), 'utf-8');
    return true;
  }

  static deleteCredentials(userDir) {
    try {
      fs.unlinkSync(`${userDir}/${LWM2M_CRED_FILE_NAME}`);
      return true;
    } catch (_) {
      return false;
    }
...
export class ResourceRepositoryBuilder {
  constructor(inputJson=[], addDefaultRepo=true, userDir='', key='') {
    if (!Array.isArray(inputJson)) {
      inputJson = [inputJson];
    }
    const credentials = CredUtils.loadCredentials(userDir, key);
    if (credentials) {
  ...
  _applyOptionsToACLObject(overlaidObjects, opts, repo) {
    ...
    overlaidObjects[LWM2M_OBJECT_ID.ACL].forEach((objectId) => {
      ...
      if (oldServerACLResource) {
        ...
        repo[`${aclUriPrefix}/2`].value[opts.serverId] = Resource.build({type: 'INTEGER', acl: 'RW', value: ACL.ALL});
      }
...
class Read extends RequestHandler {
  ...
  perform() {
    return this.client.objectStore.remoteGet(this.uris).then((resources) => {
      ...
      this.client.error(`[Read] (${this.uris}) Error Message:${err.message || ''}`, { payload: err });
...
class Write extends RequestHandler {
  ...
  constructor(client, command, payload) {
    ...
    this.params = {};
    const resources = {};
    let resoucePayload = payload.slice(8);
  ...
  perform() {
    ...
    return this.client.objectStore.remoteWrite(this.params).then(() => {
      ...
      this.client.error(`[Write] (${JSON.stringify(this.params)}) URI:${err.uri} Error Message:${err.message || ''}`, { payload: err });
...
class Execute extends RequestHandler {
  ...
  perform() {
    ...
    return this.client.objectStore.remoteExecute(uri, this.param).then(() => {
      ...
      this.client.error(`[Execute] (${uri}) Error Message:${err.message || ''}`, { payload: err });
      super.setStatus(err.status || COAP_ERROR.COAP_400_BAD_REQUEST);
      return super.perform();
    });
  }
...
class Create extends RequestHandler {
  ...
  perform() {
    ...
    return this.client.objectStore.remoteCreate(this.params).then(() => {
      ...
      this.client.error(`[Create] (${JSON.stringify(this.params)}) Error Message:${err.message || ''}`, { payload: err });
      super.setStatus(err.status || COAP_ERROR.COAP_400_BAD_REQUEST);
      return super.perform();
    });
  }
...
class Delete extends RequestHandler {
  ...
  perform() {
    ...
    return this.client.objectStore.remoteDelete(this.param).then(() => {
      ...
      this.client.error(`[Delete] (${this.param}) Error Message:${err.message || ''}`, { payload: err });
      super.setStatus(err.status || COAP_ERROR.COAP_400_BAD_REQUEST);
      return super.perform();
    });
  }
...
class Backup extends RequestHandler {
  ...
  perform() {
    ...
    return this.client.objectStore.backup(this.objectId).then(() => {
      ...
      this.client.error(`[Backup] (/${this.objectId}) Error Message:${err.message || ''}`, { payload: err });
      super.setStatus(err.status || COAP_ERROR.COAP_400_BAD_REQUEST);
      return super.perform();
    });
  }
...
class Restore extends RequestHandler {
  ...
  perform() {
    ...
    return this.client.objectStore.restore(this.objectId).then(() => {
      ...
      this.client.error(`[Restore] (/${this.objectId}) Error Message:${err.message || ''}`, { payload: err });
      super.setStatus(err.status || COAP_ERROR.COAP_400_BAD_REQUEST);
      return super.perform();
    });
  }
...
export class LwM2MClientProxy extends EventEmitter {
  ...
  constructor(opts={}, objectStore={}) {
    ...
    this.secret = opts.secret ? crypto.createHash('sha256').update(opts.secret).digest() : null;
  }
  ...
  start() {
    ...
    this.on('connected', () => {
      if (this.requestBootstrap && this.saveProvisionedConfig) {
        this.objectStore.createCredentials().then((credentials) => {
          const done = CredUtils.saveCredentials(this.userDir, this.secret, credentials);
    ...
    this.on('disconnected', (state) => {
      ...
      if (StateChanged.STATE_TABLE.STATE_BOOTSTRAP_REQUIRED === state ||
        ...
        const done = CredUtils.deleteCredentials(this.userDir);
...
export class LwM2MObjectStore {
  ...
  createCredentials() {
    ...
    let result;
    return this.get('/0/.*$').then((security) => {
      result = security;
      return this.get('/1/.*$');
    }).then((server) => {
      const objects = {};
      result.concat(server).forEach((resource) => {
