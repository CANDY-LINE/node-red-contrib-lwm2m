  ...
  class LwM2MClientNode {
    constructor(n) {
      ...
      if (this.propagateInternalEvents && lwm2m.internalEventBus && typeof(lwm2m.internalEventBus.emit) === 'function') {
        ...
        this.internalEventBus.setMaxListeners(this.internalEventBus.getMaxListeners() + 4);
        this.internalEventBus.on('object-read', (msg={}) => {
          this.objectStore.get(msg.topic).then((result) => {
            this.internalEventBus.emit('object-result', {
              id: msg.id,
              type: 'object-read',
              payload: result
            });
      ...
      this.operations = {
        ...
        shutdown() {
          if (self.internalEventBus) {
            self.internalEventBus.setMaxListeners(self.internalEventBus.getMaxListeners() - 4);
            [
              'object-read',
              'object-write',
              'object-execute',
