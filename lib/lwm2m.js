...
import {
  ResourceRepositoryBuilder, RequestHandler
} from './lwm2m-common';
...
export default function(RED) {
  ...
  if (TRACE) {
    RED.log.info(`[LwM2MClient] Start Connection`);
    RED.log.info(`[LwM2MClient] BINDING_PATH => ${BINDING_PATH}`);
    RED.log.info(`[LwM2MClient] CLIENT_PATH  => ${CLIENT_PATH}`);
  }
  ...
  class LwM2MClientNode {
    constructor(n) {
      ...
      this.operations = {
        ...
        startClient() {
          let args = ['-n', self.clientName];
          if (self.useIPv4) {
            args.push('-4');
          }
          if (self.enableDTLS) {
            args.push('-i');
            args.push(self.pskIdentity);
            args.push('-s');
            args.push(self.presharedKey);
          }
          args.push('-h');
          args.push(self.serverHost);
          args.push('-p');
          args.push(self.serverPort);

          // This function call may throw an exception on error
          self.cproc = cproc.spawn(`${CLIENT_PATH}/lwm2mclient`, args, {
            cwd: process.cwd(),
            env: process.env,
            stdio: ['pipe', 'pipe', process.stderr]
          });
          self.emit('connected');
          self.cproc.on('exit', (code) => {
            self.log(`Process Exit: pid => ${self.cproc.pid}, code => ${code}`);
            self.cproc = null;
            self.emit('disconnected');
          });
          self.cproc.stdout.on('data', (data) => {
            let lines = data.toString().split(/[\r\n]+/).filter((line) => line.trim());
            self.log(`<stdout> [Request] ${lines.length} lines, => ${lines}`);
            let body = lines[0].split(':');
            let command = body[0];
            command = command.substring(1);
            let request = RequestHandler.build(self, command, Buffer.from(body[1], 'base64'));
            self.log(`request => ${request.toJSONString()}`);
            self.log(`request.payload => [${request.payload.toString('hex')}]`);
            request.perform().then((resp) => {
              self.cproc.stdin.write(resp.payload);
              self.log(`request.response => [${request.response.toString('hex')}]`);
              self.log(`<stdout> [Response:done] ${JSON.stringify(resp)}`);
            }).catch((err) => {
              self.cproc.stdin.write(err.payload || err.toString());
              self.log(`<stdout> [Response:error] ${err.payload ? JSON.stringify(err) : err.stack}`);
            });
          });
        },
        register(node) {
      ...
      this.on('close', (done) => {
        ...
        fs.writeFile(path.join(RED.settings.userDir,
            ...
            this.objectStore.repo.serialize(), (err) => {
