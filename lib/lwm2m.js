      ...
      this.operations = {
        init() {
          return new Promise((resolve) => {
            ...
            let jsonIds = Object.keys(json);
            let initialIds = Object.keys(initialObjects).filter(id => !jsonIds.includes(id));
            initialIds.forEach((id) => {
              json[id] = initialObjects[id];
            });
            return new ResourceRepositoryBuilder(json).build(self.hideSensitiveInfo);
          }).then((repo) => {
  ...
  class LwM2MClientOutNode {
    constructor(n) {
      ...
      if (this.lwm2mClientNode) {
        ...
        this.on('input', (msg) => {
          if (this.lwm2mClientNode) {
            this.lwm2mClientNode.objectStore.write(msg.topic, msg.payload).catch((err) => {
              this.error(`lwm2m error`, { payload: err });
            });
          }
        });
