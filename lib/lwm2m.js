...
export default function(RED) {
  ...
  class LwM2MClientNode {
    constructor(n) {
      ...
      } else if (this.lazyStart) {
        this.info(`The client [${this.clientName}] will start lazily when 'start' topic message arrives.`);
      } else {
  ...
  class LwM2MClientInNode {
    constructor(n) {
      ...
      if (this.lwm2mClientNode) {
        ...
        this.on('input', (msg) => {
          if (msg && msg.topic === 'start') {
            if (this.lwm2mClientNode.lazyStart) {
              return this.lwm2mClientNode.operations.init().then(() => {
                this.lwm2mClientNode.operations.startClient();
              });
            }
            return; // ignore
          }
