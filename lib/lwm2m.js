...
export default function(RED) {
  ...
  class LwM2MClientNode {
    constructor(n) {
      ...
      this.nodes = {};
      this.objectStore = {
        repoFileName: `lwm2m_${this.clientName}_${this.serverHost}_${this.serverPort}.json`,
        repo: null,
        localWrite(uri, value) {
          if (uri) {
            this.objectStore.repo[uri] = value;
        ...
        localGet(uriRegEx) {
          if (uriRegEx) {
            // TODO
            let result = {};
            Object.keys(this.objectStore.repo).map((uri) => {
              if (uri.match(uriRegEx)) {
                result[uri] = this.objectStore.repo[uri];
              }
            });
        ...
        localDelete(uriRegEx) {
          if (uriRegEx) {
            let keysToRemove = Object.keys(this.objectStore.repo).map((uri) => {
              if (uri.match(uriRegEx)) {
                return uri;
              }
            }).filter(uri => uri);
            keysToRemove.forEach((uri) => {
              delete this.objectStore.repo[uri];
            });
            // TODO notify update to lwm2mclient
            return Promise.resolve();
          }
        ...
        remoteWrite(uri, value) {
          if (uri) {
            this.objectStore.localWrite(uri, value);
            this.emit('object-event', {
              uri: uri,
              value: value,
              eventType: 'updated',
        ...
        remoteDelete(uri) {
          if (uri) {
            this.objectStore.localDelete(uri);
            this.emit('object-event', {
              uri: uri,
              eventType: 'deleted',
              remote: true
          ...
          return new Promise((resolve, reject) => {
            fs.readFile(path.join(RED.settings.userDir,
              ...
              if (err) {
                ...
                try {
                  self.objectStore.repo = JSON.parse(data);
                } catch (_) {
                }
                resolve();
              }
      ...
      this.cproc.on('exit', () => {
        this.log(`Process Exit: pid => ${this.cproc.pid}`);
        this.cproc = null;
      });
      this.on('close', (done) => {
        ...
        fs.writeFile(path.join(RED.settings.userDir,
            ...
            JSON.stringify(this.objectStore.repo), (err) => {
          return done(err);
        });
      });
    }
    ...
    constructor(n) {
      ...
      if (this.lwm2mClientNode) {
        ...
        this.lwm2mClientNode.on('object-event', (ev) => {
          if (TRACE) {
            this.debug(`[LwM2MClientOut] <${clientName}> ${JSON.stringify(ev)}`);
          }
          this.send({
            payload: ev
          });
        ...
        this.on('input', (msg) => {
          if (TRACE) {
            this.debug(`[LwM2MClientIn] input arrived! msg=> ${JSON.stringify(msg)}`);
          }
          if (this.lwm2mClientNode) {
