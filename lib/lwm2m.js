    ...
    constructor(n) {
      ...
      this.objectStore = {
        ...
        write(uri, /* Resource */ value, /* boolean */ remote) {
          if (uri && self.objectStore.repo[uri]) {
        ...
        execute(uri, /* Buffer */ param, /* boolean */ remote) {
          if (uri && self.objectStore.repo[uri]) {
            let resource = self.objectStore.repo[uri];
        ...
        remoteWrite(/* uri-value pair object */ params) {
          if (params && Object.keys(params).length > 0) {
            return Promise.all(Object.keys(params).map((uri) => {
              ...
              return self.objectStore.write(uri, value, true);
            }));
          }
          return Promise.reject(new Error(RED._('lwm2m.error.wrongParameters')));
        },
        remoteExecute(uri, /* Buffer */ param) {
          if (uri) {
            return self.objectStore.execute(uri, param, true);
          }
          return Promise.reject(new Error(RED._('lwm2m.error.missingUri', { uri: uri })));
        },
